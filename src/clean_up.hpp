...
class AccionFinal {
 public:
  ...
  // cleanUp. Es la forma abreviadad de `this->hacerLimpieza = std::move(fn);`.
  // Con `std::move` le decimos al compilador que ya dejamos de usar el
  // parámetro `fn`, y que lo puede mover como atributo de la clase en vez de
  // copiarlo. Copiar `std::function` es caro.
  AccionFinal(std::function<void()> fn);
  // Destructor:
 ...
 private:
  ...
  // el destructor.
  std::function<void()> hacerLimpieza;
};
